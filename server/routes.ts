...
import { sendWelcomeEmail, sendContactNotification, sendContactConfirmation, sendPartnerCommissionNotification } from "./email";
import { createPayment, handleWebhook, updateMercadoPagoConfig, getMercadoPagoConfig, loadMercadoPagoConfig } from "./mercadopago";
...
import { z } from "zod";
import { db, users, partners, projects, notifications, tickets, payments, paymentStages, portfolio, referrals, projectMessages, projectFiles, projectTimeline, ticketResponses, mercadoPagoConfig, twilioConfig, paymentMethods, invoices, transactions } from "./db";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/health", async (req, res) => {
    try {
      ...
      // Test configurations
      const mpConfig = await loadMercadoPagoConfig();

      res.json({
        status: "healthy",
        database: "connected",
        timestamp: new Date().toISOString(),
        mercadopago_configured: !!(mpConfig.accessToken),
